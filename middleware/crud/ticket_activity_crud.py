...
async def get(
    ...
    for query in [x for x in params if params[x] is not None]:
        ...
        data = data.filter(get_sqlalchemy_operator(operator)(getattr(TicketActivity,attr),params[query]))
...
async def get_one(
    ...
    data = db.query(TicketActivity).options(joinedload(TicketActivity.ticket)).filter(TicketActivity.id.__eq__(id))
    return data.all()

async def post(db: Session,payload: TicketActivityInSchema):
    db_item = TicketActivity(**payload.dict())
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
